      ...
      err.error.message.forEach(err => {
        if (err.property === "lorawanSettings") {
          err.children.forEach(element => {
            errorMessages = errorMessages.concat(Object.values(element.constraints));
          });
        } else {
    ...
    if (typeof error.error === "string") {
      ...
      error.error.message.forEach(err => {
        if (err.children?.length > 0) {
          ...
          errors.errorMessages = errors.errorMessages.concat(Object.values(err.constraints));
        }
      });
    }
    return errors;
    ...
    errorChildren.forEach(error => {
      if (error.constraints) {
        errors.errorFields.push(
          error.property === "openDataDkDataset" ? "openDataDkDataset." + error.property : error.property
        );
        console.log(errors.errorFields);
        errors.errorMessages = errors.errorMessages.concat(Object.values(error.constraints));
