...
import {
  ...
  DeviceProfileResponseOne,
} from "src/app/profiles/device-profiles/device-profile.model";
...
export class DeviceProfileService {
  ...
  getOne(id: string): Observable<DeviceProfileResponseOne> {
    return this.restService.get(this.URL, {}, id).pipe(
      map((response: DeviceProfileResponseOne) => {
        response.deviceProfile.internalOrganizationName = this.sharedVariableService
          .getOrganizationInfo()
        ...
        response.deviceProfile.createdByName = this.userMinimalService.getUserNameFrom(
          response.deviceProfile.createdBy
        );
        response.deviceProfile.updatedByName = this.userMinimalService.getUserNameFrom(
          response.deviceProfile.updatedBy
        );
        return response;
      })
    );
